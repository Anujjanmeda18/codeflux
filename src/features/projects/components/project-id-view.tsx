...
const Tab = ({ label, isActive, onClick }: { label: string; isActive: boolean; onClick: () => void; }) => (
  <div
    onClick={onClick}
    className={cn(
      "flex items-center gap-2 h-full px-3 cursor-pointer text-muted-foreground border-r hover:bg-accent/30 select-none",
...
export const ProjectIdView = ({ projectId }: { projectId: Id<"projects"> }) => {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 overflow-hidden">
      <nav className="h-9 flex-shrink-0 flex items-center bg-sidebar border-b shrink-0">
