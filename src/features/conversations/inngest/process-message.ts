...
// ✅ OpenRouter config — reused for both agents
  ...
  async ({ event, step }) => {
    ...
    if (shouldGenerateTitle) {
      const titleAgent = createAgent({
        ...
        // ✅ Fast + free model for title generation (no tools needed)
        model: openai({
          ...openRouterBase,
    ...
    const codingAgent = createAgent({
      ...
      system: systemPrompt,
      // ✅ Best free model on OpenRouter for tool calling + coding
      model: openai({
        ...openRouterBase,
        model: "nvidia/nemotron-3-super-120b-a12b:free",
      }),
    ...
    if (textMessage?.type === "text") {
      assistantResponse =
        typeof textMessage.content === "string"
          ? textMessage.content
          : (textMessage.content as any[]).map((c) => c.text).join("");
    }
